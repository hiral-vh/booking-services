                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-12 mb-3">
                                    <form class="form-inline d-inline-block cus-inputwidth" method="GET" action="{{ route('business-appointment') }}">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control mr-2 mb-2" id="business" name="business">
                                                ...
                                                @foreach ($allBusinessAppointment as $value)
                                ...
                                <div class="col-12">
                                    <div class="table-responsive">
                                        <table class="table table-bordered mb-0">
                                            ...
                                            <tbody>
                                                @php
                                                $i = ($businessAppointment->currentpage() - 1) * $businessAppointment->perpage() + 1;

                                                @endphp
                                                @forelse ($businessAppointment as $value)
                                                <tr id="user_{{$value->id}}">
                                                    ...
                                                    <td width="10%">{{(isset($value->userAddress->address_line1))?$value->userAddress->address_line1:'N/A'}}<br>{{(isset($value->userAddress->street))?$value->userAddress->street:'N/A'}}<br>{{(isset($value->userAddress->city))?$value->userAddress->city:'N/A'}}<br>{{(isset($value->userAddress->post_code))?$value->userAddress->post_code:'N/A'}}</td>
                                                    ...
                                                    <td width="10%">{{(isset($value->appointment_date))?date('d-m-Y',strtotime($value->appointment_date)):'N/A'}}
                                                    </td>
                                                    <td width="10%">{{(isset($value->appointment_time))?date("h:i A", strtotime($value->appointment_time)):'N/A'}}
                                                    </td>
                                                    <td width="5%">{{(isset($value->price))?$value->price:'N/A'}}</td>
                                                    <!-- <td width="60px">
                                                ...
                                                @empty
                                                <tr>
                                                    <td colspan="11" align="center">No Bookings Available</th>
                                                </tr>
                                                @endforelse
                                            </tbody>
                                        </table>
                                        {{ $businessAppointment->appends(request()->except('page'))->links('pagination::bootstrap-4')}}
                                    </div>
                                </div>
                            </div>
                        </div> <!-- card-body -->
